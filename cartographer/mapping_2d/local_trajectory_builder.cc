 ...
 * scan-match操作，这里函数会调用real_time_correlative_scan_match来进行scan-match
 * 这个函数里面通过pose_prediction作为初始位姿给real_time_scm进行结算，用csm得到的姿态作为
 * ceres_scanmatch的初始位姿，然后通过ceres-scan_matching来进行scan-to-map的结算来得到最终的位姿.（地图为三次样条差值）
 * 然后用这个最终的位姿来更新滤波器
 * 1.滤波器给出初始位姿
 ...
 * @param pose_observation              返回的机器人观测位姿
 * @param covariance_observation        返回的机器人观测位姿的方差
 */
void LocalTrajectoryBuilder::ScanMatch(
    ...
    kalman_filter::PoseCovariance* covariance_observation)
    {
      ...
      // csm用滤波器提供的初始化进行优化，然后提供一个更好的初始值给ceres-scan-match
      ...
      /*最终通过ceres_scan_match优化来得到最终的位姿*/
...
LocalTrajectoryBuilder::AddHorizontalLaserFan(
    const common::Time time, const sensor::LaserFan3D& laser_fan)
    {
      ...
      // 目前的旋转向量是-yaw，平移向量是0,0,0   投影矩阵，数据是背包，投影到平面
      const transform::Rigid3d tracking_to_tracking_2d =
          transform::Rigid3d::Rotation(
            //绕z轴旋转-yaw度
            Eigen::Quaterniond(Eigen::AngleAxisd(-transform::GetYaw(pose_prediction), Eigen::Vector3d::UnitZ())) *
      ...
      //滤波器更新完毕之后， 帧间匹配结束之后 得到机器人的最新的估计的位姿和方差
      ...
      //这里得到的要进行插入的submap 这里的进行插入的submap一般由两个:即size()-1和size()-2
