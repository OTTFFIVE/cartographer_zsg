    ...
    transform::Rigid2d* pose_estimate) const
{
  ...
  //把激光数据旋转到和世界坐标系平行的坐标系中 (只是旋转，没有平移)
  //只所以要进行这样的旋转是因为把激光数据旋转回0度（为什么是0度,是从0度开始,负20度到正20度????）
  //因为接下来的搜索中，我们要对角度进行离散化搜索，从0度开始进行搜索.
  const sensor::PointCloud2D rotated_point_cloud =
      sensor::TransformPointCloud2D(point_cloud, transform::Rigid2d::Rotation(initial_rotation).cast<float>());
  ...
  //把激光数据经过一系列的旋转操作 得到一系列的laser_scans. 这里相当于已经把所有的不同角度的激光数据进行投影了。
  //后面枚举x,y的时候，就没必要进行投影了。这也是computing 2d slice比直接三层for循环快的原因
  //这一系列的laser_scan都是经过旋转得到的 （角度搜索空间/角度步长 = 激光数据的个数, 最后得到的激光数组保存在vector中）
  const std::vector<sensor::PointCloud2D> rotated_scans =
          GenerateRotatedScans(rotated_point_cloud, search_parameters);

  //把上面旋转得到的激光雷达数据 转换到 地图坐标系中 （现在是给激光数据加上平移）
  //经过这个函数之后，所有的激光数据的原点都和世界坐标系重合。
  //而角度也都是在世界坐标系中描述的。
  //因此对于各个不同的x_offset y_offset只需要进行激光端点的平移就可以 （Discretize表示离散化）
  ...
  //得到整个搜索空间里面的所有的候选解   Exhaustive详尽的
