    ...
    const sensor::PointCloud2D* const point_cloud)
{
  ...
  ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(    //p154
      submap_index, submap->finished_probability_grid, [=]() EXCLUDES(mutex_) {
        ComputeConstraint(submap_index, submap, scan_index, submap_trajectory,        //p202
    ...
    std::unique_ptr<OptimizationProblem::Constraint>* constraint)
{
  ...
  if (match_full_submap)
  {
    ...
    {
      return;   //没匹配上，就返回，其实回环检测的匹配肯定会得到一个阈值。
    }
  }
